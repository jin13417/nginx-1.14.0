...
static ngx_uint_t     ngx_timer_resolution;  /*时间精度，核心模块配置*/
sig_atomic_t          ngx_event_timer_alarm;

static ngx_uint_t     ngx_event_max_module;

ngx_uint_t            ngx_event_flags;
ngx_event_actions_t   ngx_event_actions;/*事件处理动作，默认是epoll事件的动作*/
...
ngx_uint_t            ngx_accept_events;           /*accept 事件*/
ngx_uint_t            ngx_accept_mutex_held;   /*是否获取accept锁*/
...
/**https://blog.csdn.net/initphp/article/details/52550360
 * Event核心模块
...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
	...
	/**
		 ...
		 **/			
                ...
                /**
		 * 1. 获取锁失败，意味着既不能让当前worker进程频繁的试图抢锁，
		 *也不能让它经过太长事件再去抢锁
		 * 2. 开启了timer_resolution时间精度，需要让ngx_process_change方法在没有新事件的
		 *时候至少等待ngx_accept_mutex_delay毫秒之后再去试图抢锁
		 * 3. 没有开启时间精度时，如果最近一个定时器事件的超时时间距离
		 *现在超过了ngx_accept_mutex_delay毫秒，也要把timer设置为ngx_accept_mutex_delay毫秒
		 * 4. 不能让ngx_process_change方法在没有新事件的时候等待的时间超过
		 *ngx_accept_mutex_delay，这会影响整个负载均衡机制
		 * 5. 如果拿到锁的进程能很快处理完accpet，而没拿到锁的一直在等待，
		 *容易造成进程忙的很忙，空的很空
		 */
...
/**https://blog.csdn.net/initphp/article/details/52550360
 * Event模块的进程初始化
 在 Nginx 的初始化启动过程中，worker 工作进程会调用事件模块的
 ngx_event_process_init 方法为每个监听套接字ngx_listening_t 分配一个 ngx_connection_t 
 连接，并设置该连接上读事件的回调方法handler 为ngx_event_accept，
 同时将读事件挂载到epoll 事件机制中等待监听套接字连接上的可读事件发生，
 到此，Nginx 就可以接收并处理来自客户端的请求。当监听套接字连接上的
 可读事件发生时，即该连接上有来自客户端发出的连接请求，则会启动
 读read事件的handler 回调方法ngx_event_accept，在ngx_event_accept 方法中调用accept() 函
 数接收来自客户端的连接请求，成功建立连接之后，ngx_event_accept 函数调用
 监听套接字ngx_listen_t上的handler 回调方法ls->handler(c)（该回调方法就是
 ngx_http_init_connection 主要用于初始化ngx_connection_t客户端连接）。ngx_http_init_connection 
 会将rev->handler的回调函数修改成： ngx_http_wait_request_handler，该回调函数主要用
 于处理read事件的数据读取。后续当有read事件上来的时候，就会回调
 ngx_http_wait_request_handler函数，而非ngx_event_accept函数。
 */
