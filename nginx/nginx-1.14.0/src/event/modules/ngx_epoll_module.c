...
/**https://blog.csdn.net/initphp/article/details/53024599
 * epoll模块命令集
 * epoll_events：这个配置项表示调用一次epoll_wait最多可以返回的事件数
 * worker_aio_requests：指明在开启异步I/O且使用io_setup系统调用初始化异
 步I/O上下文环境时，初始分配的异步I/O事件个数
 */
...
/*epooll 模块初始化
是在ngx_event_process_init 函数中事情初始化时调用到的
*/
static ngx_int_t
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (ep == -1) {
        /* 创建epoll对象，在很多Linux内核版本中，并不处理这个函数的参数   
           ...
           在linux下如果查看/proc/进程id/fd/，是能够看到这个fd的，所以在使用
...
maxevents:每次能处理的事件数；
timeout:等待I/O事件发生的超时值(单位我也不太清楚)；-1相当于阻塞，
0相当于非阻塞。一般用-1即可返回发生事件数。
epoll_wait运行的原理是等侍注册在epfd上的socket fd的事件的发生，
如果发生则将发生的sokct fd和事件类型放入到events数组中。
并 且将注册在epfd上的socket fd的事件类型给清空，所以如果
下一个循环你还要关注这个socket fd的话，则需要用
epoll_ctl(epfd,EPOLL_CTL_MOD,listenfd,&ev)来重新设置socket fd的事件类型。
这时不用EPOLL_CTL_ADD,因为socket fd并未清空，只是事件类型
清空。这一步非常重要。
