...
static ngx_command_t  ngx_core_commands[] = {
    ...
    { ngx_string("daemon"),
      ...
      0,
      offsetof(ngx_core_conf_t, daemon),/*名称在结构体偏移地址*/
      NULL },
...
static ngx_uint_t   ngx_show_configure;/*是都显示配置信息*/
static u_char      *ngx_prefix;/*Nginx的工作目录*/
static u_char      *ngx_conf_file;/*全局配置文件目录*/
static u_char      *ngx_conf_params;/*全局配置参数*/
...
/*
参考博客:
1、https://blog.csdn.net/livelylittlefish/article/details/7243718
2、https://blog.csdn.net/initphp/article/details/51832114
*/
...
main(int argc, char *const *argv)
{
    ...
    /*模块错误初始化，错误信息保存到全局数组中ngx_sys_errlist*/
    ...
    /*初始化并更新系统时间如全局变量ngx_cached_time*/
    ngx_time_init();

#if (NGX_PCRE)
    ngx_regex_init();/*正则表达式初始化malloc及释放函数*/
#endif
    /*保存进程ID 及父进程ID*/
    ngx_pid = ngx_getpid();
    ngx_parent = ngx_getppid();
    /*错误日志记录初始化全局变量ngx_prefix，打开日志文件ngx_log_file.fd*/
    ...
    /*保存命令行的所有参数，放到全局变量ngx_argv*/
    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {
        return 1;
    }
    /*将命令行解析的参数赋值到init_cycle中*/
    ...
    ngx_slab_sizes_init();
    /*ngx_add_inherited_sockets主要是继承了socket的套接字。
    主要作用是热启动的时候需要平滑过渡*/
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    if (ngx_test_config) {
        cycle->log->log_level = NGX_LOG_DEBUG;
    }

    return NGX_OK;
}

/*内存池上申请核心配置结构体并进行初始化动作*/
static void *
