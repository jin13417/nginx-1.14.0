...
typedef struct {
    ...
    u_short           len;/*name长度也就是key的长度*/
    u_char            name[1];/*某个要hash的数据(在nginx中表现为字符串)，即<key,value>中的key*/
} ngx_hash_elt_t;

/*hash结构*/
typedef struct {
    ngx_hash_elt_t  **buckets;/*hash桶的首地址,hash 桶存储的 是ngx_hash_elt_t的指针*/
    ngx_uint_t        size;/*hasht桶的个数*/
} ngx_hash_t;
...
/*主要用于创建hash*/
typedef struct {
    ngx_str_t         key;/* hash key*/
    ngx_uint_t        key_hash;/*通过hash 得到hash的key值*/
    void             *value;/*即<key,value>中的value*/
} ngx_hash_key_t;
...
typedef struct {
    ...
    ngx_hash_key_pt   key;/*hash函数指针获得hash key使用*/
