...
/*https://blog.csdn.net/initphp/article/details/51882804*/
ngx_cycle_t *
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
   ...
   /*更新时区*/
    ...
    /*初始化双向reusable_connections_queue链表*/
    ngx_queue_init(&cycle->reusable_connections_queue);

    /*初始化模块conf_ctx数组主要存储的是各个模块的配置文件结构的指针地址。*/
    ...
    * */
    for (i = 0; cycle->modules[i]; i++) {
	 ...
	 /*创建配置文件ngx_core_conf_t 结构体*/
        if (module->create_conf) {
            rv = module->create_conf(cycle); /*ngx_core_module_create_conf*/
            if (rv == NULL) {
                ngx_destroy_pool(pool);
                return NULL;
            }
   ...
   ngx_log_debug(NGX_LOG_DEBUG, old_cycle->log, 0,"[jsh] cycle->conf_file:%s",cycle->conf_file.data);
