...
ngx_conf_param(ngx_conf_t *cf)
{
    ...
    param = &cf->cycle->conf_param; /*配置参数*/
...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        /* 1.以只读的方式打开配置文件open configuration file */
	...
	/*3.缓存区数据区申请内存默认4096字节，并对缓存区初始化*/
        ...
        /*ngx_dump_config  nginx -T 会dump出配置文件信息默认不会走这个分支*/
        ngx_log_debug(NGX_LOG_DEBUG, cf->log, 0,"[jsh] ngx_dump_config:%d",ngx_dump_config);
        if (ngx_dump_config
#if (NGX_DEBUG)
            || 1
#endif
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    cf->args->nelts = 0;/*args 数据中已使用的元素个数，临时存放字节串*/
    b = cf->conf_file->buffer;/*数据缓存区大小是4096*/
    dump = cf->conf_file->dump;/*dump数据缓存区*/
    start = b->pos;/*数据缓存起始指针*/
    start_line = cf->conf_file->line;
    /*获取文件的大小*/
    file_size = ngx_file_size(&cf->conf_file->file.info);

    for ( ;; ) {
        /* buf中的数据已经处理完毕，则需要判断是否文件读取完了，
        如果没有读取完，则继续解析配置文件 */
        if (b->pos >= b->last) {
       ...
       /* 如果遇到换行符号 \n */
        if (ch == LF) {
            ...
            /* 判断改行是否是注释 如果遇到\n结尾，并且是注释，
            则设置sharp_comment = 0；当sharp_comment=1 则注释字符不处理*/
            if (sharp_comment) {
                sharp_comment = 0;
            }
        }
