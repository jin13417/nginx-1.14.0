    ...
    char *name, ngx_int_t respawn)
{
    ...
    /*首先，如果传递进来的类型大于0,则就是已经确定这个进程已经退出，
    我们就可以直接确定slot。*/
    if (respawn >= 0) {
        s = respawn;

    } else {
      /*遍历ngx_processess，从而找到空闲的slot，从而等会fork完毕后,将子进程信息放
      入全局进程信息表的相应的s*/
...
第4个参数sv[2]是接收代表两个套接口的整数数组。每一个文件描述符代表一个套接口，并且与另一个并没有区别。
    ...
    ngx_processes[s].pid = pid; /*保存子进行的id*/
